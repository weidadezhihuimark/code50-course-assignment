...
but the first letter of each subsequent word is uppercase. For instance, whereas a variable for a user’s
name might be called name, a variable for a user’s first name might be called firstName, and a variable for
a user’s preferred first name (e.g., nickname) might be called preferredFirstName.
...
def camel_to_snake(name):
    ...
    for char in name:
        # If the character is uppercase, append an underscore and its lowercase version
        if char.isupper():
